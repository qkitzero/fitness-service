...
func TestCreateOrganization(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure not group member", &organizationv1.CreateOrganizationRequest{GroupId: gid, Name: "テスト株式会社"}, true, user.ErrNotGroupMember, codes.PermissionDenied},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.callUsecase {
				...
				groupID, _ := organization.NewGroupID(gid)
				...
				mockUsecase.EXPECT().CreateOrganization(gomock.Any(), groupID, name).Return(mockOrganization, tt.createErr).Times(1)
			}
...
func TestGetOrganization(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure not group member", oid, true, user.ErrNotGroupMember, codes.PermissionDenied},
...
func TestListOrganizations(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure not group member", gid, true, user.ErrNotGroupMember, codes.PermissionDenied},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.callUsecase {
				...
				mockUsecase.EXPECT().ListOrganizations(gomock.Any(), groupID).Return(organizations, tt.listOrganizationsErr).Times(1)
			}

			handler := NewOrganizationHandler(mockUsecase)

			res, err := handler.ListOrganizations(ctx, &organizationv1.ListOrganizationsRequest{GroupId: tt.groupID})
			if got := status.Code(err); got != tt.wantCode {
				t.Errorf("expected code %v, got %v (err=%v)", tt.wantCode, got, err)
			}
			if tt.wantCode == codes.OK {
...
func TestUpdateOrganization(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure not group member", &organizationv1.UpdateOrganizationRequest{OrganizationId: oid, Name: "更新株式会社"}, true, user.ErrNotGroupMember, codes.PermissionDenied},
...
func TestToProtoOrganization(t *testing.T) {
	...
	got := toProtoOrganization(organization.NewOrganization(id, groupID, name, now, now))
