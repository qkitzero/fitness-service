...
func TestCreateCustomer(t *testing.T) {
	...
	tests := []struct {
		...
		{"success create customer", context.Background(), "test customer", "0f4a1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b", true, nil, codes.OK},
		{"failure invalid name", context.Background(), "", "0f4a1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b", false, nil, codes.InvalidArgument},
		{"failure invalid group id", context.Background(), "test customer", "", false, nil, codes.InvalidArgument},
		{"failure not group member", context.Background(), "test customer", "0f4a1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b", true, user.ErrNotGroupMember, codes.PermissionDenied},
		{"failure usecase error", context.Background(), "test customer", "0f4a1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b", true, fmt.Errorf("create customer error"), codes.Internal},
		{"failure status preserved", context.Background(), "test customer", "0f4a1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b", true, status.Error(codes.Unauthenticated, "auth"), codes.Unauthenticated},
	}
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.callUsecase {
				mockUsecase.EXPECT().CreateCustomer(tt.ctx, gomock.Any(), gomock.Any()).Return(mockCustomer, tt.createCustomerErr).Times(1)
...
func TestUpdateCustomer(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure not group member", context.Background(), "fe8c2263-bbac-4bb9-a41d-b04f5afc4425", "updated test customer", true, user.ErrNotGroupMember, codes.PermissionDenied},
