	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(mock sqlmock.Sqlmock, customer customer.Customer) {
				...
				mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO "customers" ("id","name","created_at","updated_at") VALUES ($1,$2,$3,$4)`)).
					WithArgs(customer.ID(), customer.Name(), testutil.AnyTime{}, testutil.AnyTime{}).
...
func TestFindByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(mock sqlmock.Sqlmock, customerID customer.CustomerID) {
				customerRows := sqlmock.NewRows([]string{"id", "name", "created_at", "updated_at"}).
					AddRow(customerID, "test customer", time.Now(), time.Now())
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(mock sqlmock.Sqlmock, customer customer.Customer) {
				...
				mock.ExpectExec(regexp.QuoteMeta(`UPDATE "customers" SET "name"=$1,"created_at"=$2,"updated_at"=$3 WHERE "id" = $4`)).
					WithArgs(customer.Name(), testutil.AnyTime{}, testutil.AnyTime{}, customer.ID()).
					WillReturnResult(sqlmock.NewResult(1, 1))
