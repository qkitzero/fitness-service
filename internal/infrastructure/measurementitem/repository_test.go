...
var (
	measurementItemColumns = []string{"id", "code", "name", "category", "unit", "trial_count", "side_mode", "value_type", "score_direction", "trial_aggregation", "side_aggregation", "normalization", "created_at", "updated_at"}
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				measurementItemRows := sqlmock.NewRows(measurementItemColumns).
					AddRow(muscleMassID, "muscle_mass", "筋肉量", "body_composition", "kg", 1, "none", "numeric", nil, "best", "mean", "none", createdAt, updatedAt).
					AddRow(bodyFatPercentageID, "body_fat_percentage", "体脂肪率", "body_composition", "percent", 1, "none", "numeric", nil, "best", "mean", "none", createdAt, updatedAt).
					AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt).
					AddRow(cs30ID, "cs30", "CS-30（30秒立ち座り）", "motor_function", "count", 1, "none", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt).
					AddRow(weightID, "weight", "体重", "physique", "kg", 1, "none", "numeric", nil, "best", "mean", "none", createdAt, updatedAt).
					AddRow(heightID, "height", "身長", "physique", "cm", 1, "none", "numeric", nil, "best", "mean", "none", createdAt, updatedAt).
					AddRow(pulseRateID, "pulse_rate", "脈拍", "vital", "bpm", 1, "none", "numeric", nil, "best", "mean", "none", createdAt, updatedAt).
					AddRow(bloodPressureID, "blood_pressure", "血圧", "vital", "mmHg", 1, "none", "paired", nil, "best", "mean", "none", createdAt, updatedAt)
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(multiElementID, "multi_element_item", "複数要素の測定項目", "motor_function", "count", 1, "none", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(standUpTestID, "stand_up_test", "立ち上がり", "motor_function", "level", 1, "optional_bilateral", "numeric", "higher_is_better", "best", "worst", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow("not-a-uuid", "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "GRIP STRENGTH!!", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "vitals", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kilogram", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", -1, "bilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "unilateral", "numeric", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "bogus", "higher_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "bigger_is_better", "best", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		...
		{
			name:    "failure unknown trial aggregation",
			success: false,
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "worst", "mean", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		...
		{
			name:    "failure unknown side aggregation",
			success: false,
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "median", "none", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
		...
		{
			name:    "failure unknown normalization",
			success: false,
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listSQL)).
					WillReturnRows(sqlmock.NewRows(measurementItemColumns).
						AddRow(gripStrengthID, "grip_strength", "握力", "motor_function", "kg", 2, "bilateral", "numeric", "higher_is_better", "best", "mean", "weight_ratio", createdAt, updatedAt))
				mock.ExpectQuery(regexp.QuoteMeta(elementsByItemIDSQL)).
					WithArgs(gripStrengthID).
					WillReturnRows(sqlmock.NewRows(elementColumns))
			},
